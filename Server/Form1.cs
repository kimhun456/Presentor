            ...
            while (true)
            {
                if (isConnectedServer)
                {
                    ...
                    try
                    {
                        ...
                        UpdateLogTextFromThread("Listening to server….");
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        Form2 form2 = new Form2(arry);
                        Application.Run(form2);
                        UpdateLogTextFromThread("select"+arry[form2.a].DeviceName);

                        BluetoothDeviceInfo inf = arry[form2.a];

                        mClient.Connect(inf.DeviceAddress, BluetoothService.SerialPort);
                        
                        UpdateLogTextFromThread("Conneting Success to : " + mClient.RemoteMachineName);
                        ...
                        {
                            BluetoothDeviceInfo inf = dialog.SelectedDevice;
                            
                            mClient.Connect(inf.DeviceAddress, BluetoothService.SerialPort);
                            UpdateLogTextFromThread("Conneting Success to : " + inf.DeviceName);
                            
                            isConnected = mClient.Connected;
                            UpdateLogTextFromThread("A Bluetooth Device Connected!");
                        }
                        */
                    }
